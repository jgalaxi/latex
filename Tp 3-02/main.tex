...
Antes de comenzar a explicar cómo se obtiene el arrabio, el acero, la fundición y qué hornos están asociados con la obtención de cada uno, primero vamos a definir una serie de términos que son importantes para entender de qué estamos hablando. También cabe destacar que sólo nos concentraremos en la obtención de los tres materiales mencionados, sus materias primas y la descripción de los hornos correspondientes a la fabricación, dejando de lado cuestiones que tienen que ver con la minería y la obtención de la materia prima.
...
El arrabio es el producto metálico intermedio que se obtiene de la reducción de los minerales de hierro en el alto horno (En la sección \hyperref[alto_horno]{2.2} se explicará más a detalle). Es una aleación de hierro con un alto contenido de carbono, normalmente entre \SI{3}{\percent} y \SI{4,5}{\percent}, y que además incluye impurezas como silicio, fósforo, manganeso y azufre. El nombre “arrabio” proviene del antiguo término “hierro bruto”, ya que se trata de un material inicial que no tiene aplicación directa salvo como materia prima para fabricar acero o fundición.
...
\end{itemize}
 ...
 La fundición es una aleación de hierro y carbono cuyo contenido de carbono se sitúa entre \SI{2,1}{\percent} y \SI{4,5}{\percent}. Se obtiene a partir de la refundición del arrabio, generalmente en hornos como el cubilote, el crisol o el arco eléctrico. A diferencia del acero, la fundición conserva un mayor porcentaje de C, lo que modifica sus propiedades y determina que sea un material excelente para ser colado en moldes, de ahí su nombre.
...
La mena es aquel mineral que contiene suficiente cantidad de hierro para que su extracción y procesamiento resulten económicamente rentables. Por ejemplo, la magnetita (\ce{Fe3O4}) y la hematita (\ce{Fe2O3}) son menas muy ricas, con porcentajes de hierro de aproximadamente \SI{72}{\percent} y \SI{69}{\percent} respectivamente. También se encuentran otros minerales como la siderita (\ce{FeCO3}), que aunque contiene menos hierro ($\approx$ \SI{58}{\percent}) se aprecia por su bajo contenido en P y S, lo que la hace útil en procesos de afino. En otros casos aparece la pirita (\ce{FeS2}), pero esta se descarta como fuente de hierro debido a su elevado contenido en azufre.
...
Una planta de coque produce coque metalúrgico desde una mezcla de carbones. Se produce calentando el carbón a \SI{1250}{\celsius} en una atmósfera libre de oxígeno. El coque producido se compone de más de un \SI{90}{\percent} de C, que se emplea como suministro energético y agente químico en el alto horno.

\section{Obtención del Arrabio.}

Una vez que la mena es seleccionada y mediante un proceso es preparada, se da inicio al proceso de reducción para obtener el primer producto metálico: el arrabio. Este se fabrica en el alto horno, cuyo funcionamiento es continuo, lo que significa que puede operar durante años sin interrupciones, recibiendo constantemente mineral de hierro, coque y caliza por la parte superior. El coque actúa simultáneamente como combustible y reductor químico, ya que al reaccionar con el aire caliente inyectado por las toberas en la base del horno genera \ce{CO} (esto se aprecia en la \autoref{fig:alto_horno}). Este gas asciende por la columna de carga y reduce progresivamente los óxidos de hierro presentes en el mineral. Mientras tanto, el fundente (caliza) se descompone en óxido de calcio, que se combina con impurezas como la sílice para formar escoria. De este modo, en la parte baja del horno se acumula una capa de hierro fundido, llamada arrabio, cubierta por la escoria flotante que lo protege de la oxidación. Periódicamente, tanto el hierro como la escoria se extraen a través de orificios llamados piqueras. 

El arrabio resultante contiene entre \SIrange[range-units=single]{3}{4,5}{\percent} de carbono, además de fósforo, azufre, manganeso y silicio, lo que lo convierte en un material frágil y de uso limitado en su forma directa.
...
Por ende, la obtención del arrabio en alto horno es el método tradicional y principal, donde el mineral se funde y se reduce con \ce{CO} generado por la combustión del coque, siendo el producto resultante el arrabio líquido. Este proceso se llama reducción indirecta porque el oxígeno del mineral se elimina principalmente gracias al monóxido de carbono como agente reductor intermedio, no directamente con el carbono sólido.

También existen otros métodos modernos que se llaman de reducción directa (DRI\footnote{Direct Reduced Iron, en español: hierro reducido directamente}), aquí no se usa alto horno ni coque, sino gas natural reformado (\ce{CO + H2}) o carbón pulverizado, donde el mineral se reduce en estado sólido, sin llegar a fundirse. En este caso el producto no es arrabio líquido, sino hierro esponja (o DRI), que luego se funde en hornos de arco eléctrico para hacer acero. Este proceso sí se llama reducción directa, porque el oxígeno se elimina directamente con \ce{CO + H2} sin pasar por una etapa de fusión.
...
\begin{itemize}
    \item \textbf{Carga:} mineral de hierro (\SI{2}{\tonne}), coque (\SI{1}{\tonne}), caliza (\SI{0,5}{\tonne}) y aire (\SI{4}{\tonne}).
    \item \textbf{Producto:} arrabio (\SI{1}{\tonne}, con composición típica: Fe \SI{93,7}{\percent}; C \SI{4,5}{\percent}; Mn \SI{0,4}{\percent}; Si \SI{0,45}{\percent}; P \SI{0,11}{\percent}; S \SI{0,025}{\percent}), escoria (\SI{0,5}{\tonne}) y gases (\SI{6}{\tonne}).
    \item \textbf{Temperaturas:} desde \SI{200}{\celsius} en la parte superior hasta \SI{1500}{\celsius} en la zona de fusión.
    \item \textbf{Eficiencia:} se mejora con aire precalentado (\SI{1030}{\celsius}) y enriquecido en oxígeno, reduciendo pérdidas térmicas.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Uso:} reducción de minerales de hierro en estado sólido para producir arrabio líquido. Es un horno continuo de gran capacidad (hasta \SI{5000}{\tonne\per\day}), con estructura cilíndrica refractaria, toberas para inyección de aire caliente y regeneradores para precalentar el aire.
    \item \textbf{Características:} es una estructura monumental de forma cilíndrica, con una altura de \SIrange[range-units=single]{20}{40}{\metre}, revestida con ladrillos refractarios capaces de soportar temperaturas superiores a \SI{1600}{\celsius}, funciona a contracorriente (carga por arriba, gases por debajo). Asociado a procesos como sinterización de minerales y producción de coque. Su funcionamiento es continuo, lo que significa que puede operar durante años sin interrupciones, recibiendo constantemente mineral de hierro, coque y caliza.
\end{itemize}
...
La fundición se obtiene a partir de la refusión del arrabio en hornos específicos, a los cuales se le puede añadir chatarra metálica y fundentes según las propiedades buscadas en el producto final. El proceso comienza con la carga del arrabio sólido (en lingotes) o líquido proveniente del alto horno, junto con coque como combustible y piedra caliza como fundente (en el caso del cubilote). La función del coque es doble: aportar calor por combustión e introducir \ce{CO}, que actúa como reductor secundario. En los hornos de arco eléctrico o crisol, la energía de fusión proviene de la electricidad o del calor transmitido indirectamente, lo que brinda un mayor control químico.

Durante la fusión, la temperatura se eleva entre \SIrange[range-phrase = { y }]{1350}{1500}{\celsius}, lo suficiente para mantener el hierro en estado líquido y permitir la formación de escoria. A diferencia del acero, en el proceso de obtención de fundición no se busca reducir el carbono, sino mantener un contenido elevado (de \SIrange[range-units=single]{2,1}{4,5}{\percent}). De hecho, en muchos casos se añade carbono extra (por ejemplo, en forma de coque o aditivos carbonosos) para ajustar la composición. Asimismo, es frecuente introducir elementos de aleación (silicio, manganeso, níquel, cromo, magnesio) que modifican la microestructura y dan lugar a fundiciones con propiedades específicas.
...
\begin{itemize}
    ...
    \item Si el enfriamiento es rápido, el carbono queda combinado en forma de \ce{Fe3C}, originando la fundición blanca.
    \item Si la fundición blanca se somete a un recocido prolongado (varios días a temperaturas entre \SIrange[range-phrase = { y }, range-units=single]{900}{950}{\celsius}), el carbono combinado se descompone y parte de él precipita como grafito irregular, transformándola en fundición maleable.
    \item Si durante la fusión se añade Mg o Ce, el grafito se nuclea en forma de esferas, lo que genera la fundición nodular o esferoidal, con una microestructura mucho más tenaz y resistente.
\end{itemize}

Por lo tanto, la obtención de fundición implica no solo la simple fusión del arrabio, si no también el control del horno, de la atmósfera de fusión, de los aditivos y de la velocidad de enfriamiento, lo que permite orientar la microestructura hacia grafito laminar, cementita, grafito esferoidal o combinaciones intermedias. Este control microestructural es lo que explica la gran variedad de fundiciones y su amplio rango de aplicaciones industriales.
...
\begin{itemize}
    ...
    \item \textbf{Características:} está constituido de un crisol hecho de material refractario, como grafito, hecho para contener el material que se va a fundir y el cual soporta temperaturas extremadamente altas. El crisol se calienta mediante aplicación de calor desde el exterior, que puede ser mediante combustibles sólidos como coque, gaseosos como gas natural, propano, electricidad u otros medios.
\end{itemize}
...
Un convertidor es un gran recipiente en forma de pera, revestido interiormente de material refractario, cuyo fondo está perforado. Mientras se vierte la colada líquida, el convertidor se mantiene en posición horizontal para evitar que el líquido alcance los orificios del fondo. Una vez que se ha completado la carga, el convertidor se endereza, al mismo tiempo que comienza el soplado de aire a 2-3 atm de presión a través de los orificios del fondo. El oxígeno del aire oxida al hierro formando \ce{FeO}, este se disuelve y oxida al silicio y al manganeso. Los óxidos formados reaccionan con el \ce{SiO2} y con el revestimiento formando una escoria que flota sobre el material fundido. Luego, comienza la oxidación del carbono. Cuando el \ce{CO} llega a la atmósfera se observan llamaradas de \SIrange[range-units=single]{7}{9}{\metre} en la boca del convertidor, alcanzando en el interior una temperatura de \SI{1600}{\celsius}. Una rápida disminución en el largo de la llama revela que la descarburación ha terminado. El proceso en el convertidor dura unos \SI{20}{\minute} durante los que se pueden afinar de \SIrange[range-units=single]{10}{25}{\tonne} de arrabio. Esta rapidez no permite un control muy exacto del proceso y, por lo tanto, de la composición final del acero.
...
\begin{itemize}
    \item \textbf{Carga típica:} arrabio (\SI{93}{\percent} Fe; \SI{4}{\percent} C; \SIrange[range-units=single]{0,5}{2}{\percent} Si; \SI{1}{\percent} Mn; \SIrange[range-units=single]{2}{0,1}{\percent} P; \SI{0,05}{\percent} S) + chatarra + fundentes.
    ...
    \item \textbf{Producto:} acero con Fe al \SI{98}{\percent}; C \SIrange[range-units=single]{0,05}{1,5}{\percent}; Si \SIrange[range-units=single]{0,5}{2}{\percent}; Mn \SIrange[range-units=single]{0,3}{0,6}{\percent}; P/S < \SI{0,05}{\percent}.
\end{itemize}
...
\begin{itemize}
    ...
    Sobre el arrabio fundido se hace incidir un chorro de oxígeno puro insuflado en sentido vertical y a presión. Es un proceso muy rápido que requiere un control automatizado de las cargas de arrabio y de fundente que se utilizan, y de la presión y el caudal del oxígeno. Al insuflar oxígeno sobre el arrabio se genera óxido ferroso (\ce{FeO}), el cual reacciona con las impurezas y forma óxidos, con los que se eliminan estas impurezas. Luego, se añade rápidamente fundente y se sigue insuflando oxígeno que forma monóxido de carbono (\ce{CO}) y dióxido de carbono (\ce{CO2}), de manera que se reduce el contenido de C, hasta llegar al grado de composición deseado. Este procedimiento es uno de los más modernos y el más utilizado en la actualidad porque permite obtener aceros de muy buena calidad, es relativamente sencillo y de bajo costo.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Características:} 
    El proceso es similar al del LD, con la diferencia de que se introduce una mezcla de oxígeno y nitrógeno (aire); el nitrógeno produce nitruros de hierro que, a pesar de estar presentes en pequeñas cantidades, proporcionan dureza y fragilidad al acero. Es un método antiguo que prácticamente ya no se utiliza. El convertidor Bessemer o de pared ácida se usa cuando no existen impurezas de azufre o fósforo.
\end{itemize}

\textbf{Tipo: Convertidores Thomas.}

\begin{itemize}
    \item \textbf{Características:} es una modificación del convertidor Bessemer. Se diferencia porque la cubata está revestida con una mezcla  de materiales de PH básico (al contrario que los Bessemer) y alquitrán, que permitía eliminar del acero el fósforo presente en el mineral de hierro. El fósforo contenido en el arrabio se combina con la caliza presente en el mineral de hierro.
\end{itemize}
...
\begin{figure}[H]
    ...
    \begin{subfigure}{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{Inagenes para latex/simer-martin.png}
        \subcaption{Horno Siemens-Marin}
        \label{Siemens2}
    \end{subfigure}
...
\subsection{Diferencias entre los tipos de hornos.}

\begin{table}[h]
    \centering
    \begin{tabular}{cccccccc}
    \toprule
    \textbf{Clase} & \textbf{C \%} & \textbf{Mn \%} & \textbf{Si \%} & \textbf{P \%} & \textbf{S \%} & \textbf{R (Kg/mm$^2$)} & \textbf{A \%} \\
    \midrule
    B-1 & 0,1 $\pm$ 0,05 & 0,35 $\pm$ 0,10 & $<$ 0,10 &               &               & 41 & 28 \\
    B-2 & 0,2 $\pm$ 0,05 & 0,45 $\pm$ 0,15 & $<$ 0,20 &               &               & 49 & 23 \\
    B-3 & 0,3 $\pm$ 0,05 & 0,55 $\pm$ 0,15 &           &               &               & 57 & 20 \\
    B-4 & 0,4 $\pm$ 0,05 & 0,65 $\pm$ 0,15 & $<$ 0,30 & $<$ 0,07      &               & 66 & 17 \\
    B-5 & 0,5 $\pm$ 0,05 & 0,70 $\pm$ 0,15 &           &               &               & 75 & 14 \\
    B-6 & 0,6 $\pm$ 0,05 & 0,75 $\pm$ 0,15 &           &               &               & 84 &  9 \\
    B-7 & 0,7 $\pm$ 0,05 & 0,80 $\pm$ 0,15 &           &               &               & 92 &  7 \\
    \bottomrule
    \end{tabular}
    \caption{Aceros al carbono fabricados en convertidor Bessemer (\%P + S < 0,12\%)}
    \label{tab:AceBessemer}
\end{table}
...
\toprule
\textbf{Clase} & \textbf{C \%} & \textbf{Mn \%} & \textbf{Si \%} & \textbf{P \%} & \textbf{S \%} & \textbf{Estado} & \textbf{R (Kg/mm$^2$)} & \textbf{A \%} \\
\midrule
\multirow{2}{*}{S-1} & \multirow{2}{*}{0,1 $\pm$ 0,05} & \multirow{2}{*}{0,35 $\pm$ 0,05} & \multirow{2}{*}{$<$ 0,10} & & & Normalizado & 41 & 29 \\
                     &                                 &                                 &                          & & & Recocido   & 39 & 30 \\
\midrule
\multirow{2}{*}{S-2} & \multirow{2}{*}{0,2 $\pm$ 0,05} & \multirow{2}{*}{0,40 $\pm$ 0,10} & \multirow{2}{*}{$<$ 0,20} & & & Normalizado & 49 & 25 \\
                     &                                 &                                 &                          & & & Recocido   & 43 & 28 \\
\midrule
\multirow{2}{*}{S-3} & \multirow{2}{*}{0,3 $\pm$ 0,05} & \multirow{2}{*}{0,45 $\pm$ 0,10} &                          & & & Normalizado & 57 & 20 \\
                     &                                 &                                 &                          & & & Recocido   & 50 & 25 \\
\midrule
\multirow{2}{*}{S-4} & \multirow{2}{*}{0,4 $\pm$ 0,05} & \multirow{2}{*}{0,50 $\pm$ 0,15} & \multirow{2}{*}{$<$ 0,30} & \multirow{2}{*}{$<$ 0,04} & \multirow{2}{*}{$<$ 0,06} & Normalizado & 65 & 17 \\
                     &                                 &                                 &                          &                          &                          & Recocido   & 55 & 23 \\
\midrule
\multirow{2}{*}{S-5} & \multirow{2}{*}{0,5 $\pm$ 0,05} & \multirow{2}{*}{0,55 $\pm$ 0,20} &                          & & & Normalizado & 73 & 12 \\
                     &                                 &                                 &                          & & & Recocido   & 60 & 20 \\
\midrule
\multirow{2}{*}{S-6} & \multirow{2}{*}{0,6 $\pm$ 0,05} & \multirow{2}{*}{0,70 $\pm$ 0,20} &                          & & & Normalizado & 82 & 15 \\
                     &                                 &                                 &                          & & & Recocido   & 65 & 18 \\
\midrule
\multirow{2}{*}{S-7} & \multirow{2}{*}{0,7 $\pm$ 0,05} & \multirow{2}{*}{0,75 $\pm$ 0,25} &                          & & & Normalizado & 89 & 13 \\
                     &                                 &                                 &                          & & & Recocido   & 70 & 17 \\
\bottomrule
\end{tabular}%
}
\caption{Aceros al carbono fabricados en hornos Siemens (\%P + S < 0,09\%)}
\label{tab:AceSiemens}
\end{table}
...
\textbf{Clase} & \textbf{C \%} & \textbf{Mn \%} & \textbf{Si \%} & \textbf{P \%} & \textbf{S \%} & \textbf{Estado} & \textbf{R (Kg/mm$^2$)} & \textbf{A \%} \\
\midrule
\multirow{2}{*}{F-151} & \multirow{2}{*}{0,08 $-$ 0,12} & \multirow{2}{*}{0,30 $-$ 0,40} &                         & & & Normalizado & 41 & 29 \\
                     &                                 &                                 &                          & & & Recocido   & 39 & 30 \\
\midrule
\multirow{2}{*}{F-111} & \multirow{2}{*}{0,10 $-$ 0,20} & \multirow{2}{*}{0,30 $-$ 0,50} &                           & & & Normalizado & 45 & 27 \\
                     &                                 &                                 &                          & & & Recocido   & 41 & 29 \\
\midrule
\multirow{2}{*}{F-112} & \multirow{2}{*}{0,20 $-$ 0,30} & \multirow{2}{*}{0,40 $-$ 0,70} &                          & & & Normalizado & 52 & 24 \\
                     &                                 &                                 &                          & & & Recocido   & 48 & 25 \\
\midrule
\multirow{2}{*}{F-113} & \multirow{2}{*}{0,30 $-$ 0,40} & \multirow{2}{*}{0,40 $-$ 0,70} & \multirow{2}{*}{0,15 $-$ 0,30} & \multirow{2}{*}{$<$ 0,03} & \multirow{2}{*}{$<$ 0,03} & Normalizado & 60 & 20 \\
                     &                                 &                                 &                          &                          &                          & Recocido   & 53 & 22 \\
\midrule
\multirow{2}{*}{F-114} & \multirow{2}{*}{0,40 $-$ 0,50} & \multirow{2}{*}{0,40 $-$ 0,70} &                          & & & Normalizado & 69 & 18 \\
                     &                                 &                                 &                          & & & Recocido   & 58 & 21 \\
\midrule
\multirow{2}{*}{F-115} & \multirow{2}{*}{0,50 $-$ 0,60} & \multirow{2}{*}{0,50 $-$ 0,80} &                          & & & Normalizado & 77 & 16 \\
                     &                                 &                                 &                          & & & Recocido   & 62 & 19 \\
\bottomrule
\end{tabular}%
}
\caption{Aceros al carbono fabricados en hornos electricos (\%P + S < 0,09\%)}
\label{tab:AceElectricos}
\end{table}

Como se observa en las \autoref{tab:AceBessemer}, \autoref{tab:AceSiemens} y \autoref{tab:AceElectricos}, donde se presentan las composiciones y propiedades mecánicas de distintos aceros obtenidos en cada tipo de horno, la principal diferencia entre ellos radica en su grado de pureza, asociado principalmente al contenido de fósforo y azufre. Estos elementos influyen de manera directa en las propiedades mecánicas: los aceros producidos en hornos Bessemer presentan mayor resistencia, pero menor ductilidad que el resto. En contraste, los aceros elaborados en hornos Siemens y eléctricos se consideran de mejor calidad debido a su mayor pureza. Finalmente, los aceros obtenidos en hornos LD alcanzan una calidad superior a la de los Bessemer, con propiedades comparables a las de los Siemens e incluso a las de los hornos eléctricos.

\section{Conclusión.}
La comparación de estos hornos y procesos muestra cómo la siderurgia evolucionó desde sistemas rudimentarios hasta complejos y altamente eficientes hornos. El alto horno es esencial para obtener arrabio a gran escala; los hornos de cubilote, crisol y arco eléctrico diversifican la producción de fundiciones; y los convertidores Bessemer, Thomas, Siemens-Martin, LD y hornos de arco eléctrico marcan la historia y la actualidad de la producción de acero. Cada horno tiene un papel particular dentro de la cadena de producción: el alto horno para la reducción de minerales, el cubilote para fundiciones económicas, el crisol para aleaciones especiales, el arco eléctrico para fundiciones y aceros de alta precisión, y los convertidores para el afino masivo del arrabio en acero.
